...
const normalizeOperations = (spec: OpenApiSpec): NormalizedOperation[] => {
  ...
  for (const [path, pathItem] of Object.entries(spec.paths)) {
    for (const method of HTTP_METHODS) {
      ...
      const isObjectBody = hasBody && (bodySchema?.type === 'object' || (!bodySchema?.type && !!bodySchema?.properties));
 ...
 * Regenerate with: pnpm gen:client (or pnpm generate to refresh the spec first).
 */
/* eslint-disable */
...
`;
  ...
  for (const tag of tags) {
    ...
    const methods = byTag.get(tag)!.map((op) => {
      ...
      const paramsArg = op.paramsRequired
